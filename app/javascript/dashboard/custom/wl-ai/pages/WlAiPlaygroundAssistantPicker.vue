...
defineOptions({
  name: 'WlAiPlaygroundAssistantPicker',
});
...
const selectOptions = computed(() =>
  assistants.value.map(a => ({
    value: a.id,
    label: a.name?.trim() || t('WL_AI.PLAYGROUND_PICK.UNNAMED', { id: a.id }),
  }))
);
...
const load = async () => {
  ...
  try {
    ...
    useAlert(t('WL_AI.PLAYGROUND_PICK.LOAD_ERROR'));
    assistants.value = [];
    selectedAssistantId.value = '';
  } finally {
...
const goToPlayground = () => {
  ...
  router.push(
    accountScopedRoute('wl_ai_playground_index', { assistantId: String(id) })
  );
};
...
<template>
  ...
  >
    <Dialog
      ...
      type="edit"
      width="md"
      :title="t('WL_AI.PLAYGROUND_PICK.TITLE')"
      :description="t('WL_AI.PLAYGROUND_PICK.DESCRIPTION')"
      :confirm-button-label="t('WL_AI.PLAYGROUND_PICK.CONTINUE')"
      :cancel-button-label="t('WL_AI.PLAYGROUND_PICK.CANCEL')"
