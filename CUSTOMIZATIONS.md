...
| WL Bot Flows — DB/models | `db/migrate/2026051812000*_create_wl_bot_*.rb`, `app/models/wl_bot_flow.rb`, `wl_bot_session.rb`, `wl_bot_execution_log.rb`, `app/models/account.rb` | NEW + PATCH | Fluxos visuais, sessões por conversa e logs de execução. |
| WL Bot Flows — runtime | `config/initializers/wl_bot_flows_listener.rb`, `app/listeners/wl_bot_flows/agent_bot_listener_extension.rb`, `app/services/wl_bot_flows/**`, `app/jobs/wl_bot_flows/**` | NEW | Engine determinística via `AgentBot` + prepend no `AgentBotListener` (sem HTTP loopback). Opt-in `WL_BOT_FLOWS_ENABLED` (default `true`). |
| WL Bot Flows — API | `config/routes.rb`, `app/controllers/api/v1/accounts/wl_bot_flows/**`, `app/views/api/v1/accounts/wl_bot_flows/**` | NEW | CRUD, publish/pause/duplicate, execution logs. |
| WL Bot Flows — UI | `app/javascript/dashboard/custom/wl-bot-flows/**`, `app/javascript/dashboard/api/wlBotFlows/flows.js`, `dashboard.routes.js`, `Sidebar.vue`, `sidebarConfig.js`, `package.json`, `i18n/locale/en/wlBotFlows.json`, `settings.json` | NEW + PATCH | Builder Vue Flow (lista, editor, logs). |
| WL Bot Flows — i18n backend | `config/locales/en.yml` | PATCH | Erros de publicação (exclusividade WL AI / Captain / Dialogflow). |
| WL Bot Flows — i18n pt_BR (fork) | `app/javascript/dashboard/i18n/locale/pt_BR/wlBotFlows.json`, `pt_BR/settings.json`, `pt_BR/index.js`, `config/locales/pt_BR.yml` | NEW + PATCH | Traduções do módulo para locale padrão do fork. |
| WL Bot Flows — specs | `spec/factories/wl_bot_flows.rb`, `spec/services/wl_bot_flows/**`, `spec/models/wl_bot_flow_publish_spec.rb`, `spec/listeners/wl_bot_flows/**` | NEW | Validator, runtime, publish, listener. |
...
- **WL Bot Flows vs WL AI:** uma inbox não deve ter assistente WL AI ligado e fluxo publicado ao mesmo tempo — `WlBotFlow#publish!` bloqueia se `inbox.wl_ai_assistant` existir.
- **WL Bot Flows vs Captain:** não enfileirar runtime se `inbox.captain_active?` (Enterprise).
- **WL Bot Flows — navegação (Fase A):** `entry_group_id` define o grupo inicial (editável no header do editor). Após a última ação de um grupo, o runtime segue `group.next_group_id` se definido; caso contrário `finish_session!`. Grupos vazios seguem `next_group_id` imediatamente. No canvas, o handle `next` (rodapé “Then”) sincroniza com `next_group_id`; arestas `next` não são persistidas em `edges[]` (só condicionais). Migração automática de arestas legado `source` → `next_group_id` ao abrir o editor.
- **WL Bot Flows — condicional:** ação `condition` com ramos (`rules[]` + `fallback_group_id`). Cada ramo tem comparativos (`source`: `session_variable`, `contact`, `conversation`, `inbox`, `datetime`; `attribute`, `operator`, `value`) e destino via `target_group_id`. Fontes adicionais: `inbox.working_now` (horário de expediente), `conversation.labels` (etiquetas, operadores `includes`/`not_includes`), `contact`/`conversation` com `custom_attribute:<key>`, `datetime` com `hour`/`day_of_week`/`date`. No canvas, handles `condition:{actionId}:{ruleId}` e `condition:{actionId}:fallback` sincronizam com `target_group_id`. Fluxo típico: `ask` (save_as) → `condition`. Publicar exige `fallback_group_id` e regras completas (`Validator` com `publish: true`).
- **WL Bot Flows — Pergunta (Fase B1):** ação `ask` com subtipo `text` (texto livre), `buttons` ou `list` (botões/lista interativos). Envia mensagem e aguarda resposta do contato. Campos: `text`, `subtype`, `options[]` ({`id`, `label`, `target_group_id`}), `save_as`, `timeout`, `timeout_group_id`, `invalid_response_group_id`. No canvas: handle `ask:{actionId}:{optionId}` por opção, `ask:{actionId}:timeout`, `ask:{actionId}:invalid`. Timeout com ramo alternativo gerido pelo `SessionTimeoutJob`. Respostas fora das opções → `invalid_response_group_id`.
- **WL Bot Flows — Webhook síncrono (Fase B3):** `send_webhook` com `wait_response: true` faz HTTP síncrono (via Faraday, default 20s). Resposta 2xx → `success_group_id` ou `:continue`. Não-2xx/timeout → `failure_group_id` ou `:failed`. Sem `wait_response` (default `false`) → fire-and-forget (comportamento anterior). No canvas: handles `webhook:{actionId}:success` e `webhook:{actionId}:failure`.
- **Agent bot na inbox:** publicar fluxo ativa `agent_bot_inbox` → auto-reply WL AI deixa de correr (`other_bot_integrations_active?`), mas o vínculo `wl_ai_assistant` deve ser removido antes de publicar.
- **Runtime:** mensagens outgoing do próprio `AgentBot` são ignoradas; executar só `published_json`.
- **Vue Flow:** dependências `@vue-flow/*` em `package.json` — correr `pnpm install` após pull. Editor carrega Vue Flow em chunk separado (lazy) para não inflar o build do `dashboard.js`.
- **Docker / Vite OOM:** com `docker compose`, o serviço `vite` deve estar a correr; se o Rails não o alcançar, `autoBuild` compila no container `rails` e pode falhar por memória. `docker-compose.yaml` define `NODE_OPTIONS=--max-old-space-size=4096` em `rails` e `vite`. Após alterar compose: `docker compose up -d vite` e `docker compose restart rails`.
